...
import tensorflow as tf # As I have installed tf 2.0 but the code is written on tf 1.7
...
if K>1:
    restoreWeights=True
    restoreFromModel='04Jul_1157pm_5L_10K_100E_AG_1BS'

if restoreWeights:
    wts=sf.getWeights('savedModelsR16/'+restoreFromModel)
#--------------------------------------------------------------------------
#%%Generate a meaningful filename to save the trainined models for testing
print ('*************************************************')
start_time=time.time()
saveDir='savedModelsR16/'
cwd=os.getcwd()
directory=saveDir+datetime.now().strftime("%d%b_%I%M%P_")+ \
 str(nLayers)+'L_'+str(K)+'K_'+str(epochs)+'E_'+gradientMethod +'_'+str(batchSize)+'BS'
...
trnOrg,trnAtb,trnCsm,trnMask,_ = sf.getData('training')
trnOrg,trnAtb=sf.c2r(trnOrg),sf.c2r(trnAtb)


R = trnMask.shape[0]*trnMask.shape[1]*trnMask.shape[2]/np.sum(trnMask)
print('R = ',R)
#%%
tf.reset_default_graph()
