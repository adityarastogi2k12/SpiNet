...
def getData(trnTst='testing',num=100,sigma=.01):
    ...
    with h5.File(filename) as f:
        if trnTst=='training':
            ...
            mask = sio.loadmat('R16.mat')['U1']
        else:
            org,csm,mask=f['tstOrg'][num],f['tstCsm'][num],f['tstMask'][num]
            mask = sio.loadmat('R16.mat')['U1'][num]
            na=np.newaxis
    ...
    tic()
    atb,b=generateUndersampled(org,csm,mask,sigma)
    toc()
    print('Successfully undersampled data!')
    if trnTst=='testing':
        atb=c2r(atb)
    return org,atb,csm,mask,b
...
def getTestingData(sigma = 0.01):
    ...
    with h5.File(filename,'r') as f:
        org,csm,_=f['tstOrg'][:],f['tstCsm'][:],f['tstMask'][:]
        mask = sio.loadmat('R16.mat')['U1'][:org.shape[0]]
    toc()
    print('Successfully read the data from file!')
    print('Now doing undersampling....')
    tic()
